...
@if(isset($create) && $create == false)
    <h4 class="col-lg-6 col-md-6 col-sm-6  no-padding"></h4>
...
@elseif(empty($create ) || $create  == true)
  ...
  <h4 class="col-lg-6 col-md-6 col-sm-6 no-padding">
      <a class="btn btn-primary btn-lg" href='/{{$path}}/create'>Create New {{$page_title}}</a>
  </h4>
  <h4 class="col-lg-6 col-md-6 col-sm-6 text-right">Total Result:@if(isset($total_result)) {!! $total_result !!} @else {!! count($array)!!} @endif</h4>
  @else
        ...
        @foreach($header as $row)
          ...
          @if($row['canView'] == true)
            ...
            @elseif($row['type'] == 'link')
              <td><p><a  href='{{$row['path']}}/{!! $record->{$row['dbName']} !!}'>@if($record->{$row['message']}){!! $record->{$row['message']} !!} @else {{$row['message']}} @endif</a></p></td>
            <!-- End link -->
...
{{--                              {{ parentRole($record->id) }}--}}
            ...
            <!-- Form Input for external_link -->
                <td>
                    <a TARGET="_blank" href="{!! $row['path'].$record->{$row['dbName']} !!}" >{!! $record->{$row['dbName']} !!}</a>
                </td>
            ...
            <!-- Form Input for image -->
                <td>
                    <p style="max-width: 75px;">
                        <a data-fancybox href="{!! $row['path'].$record->{$row['dbName']} !!}" ><img class="img-responsive thumbnail" src="{!! $row['path'].$record->{$row['dbName']} !!}" alt=""></a>
                    </p>
                </td>
       ...
       <td class='action'>
          @if(roles($user_id) != 'subscriber' && roles($user_id) != 'contributor')
            @if(isset($view_page) && $view_page == true)
               <a class='view_page' target="_blank" title="View" href='/{{$path}}/{{$record->$table_pk}}'>
                   <span class="glyphicon glyphicon-eye-open"></span>
               </a>
            @endif
            @if(isset($edit) && $edit == false)
            @else
              <a class='edit' title="Edit" href='/{{$path}}/{{$record->$table_pk}}/edit'>
                <span class="glyphicon glyphicon-pencil"></span>
              </a>
            @endif
            @if(isset($delete) && $delete == false)
            @else
              <a class="delete" title="Delete" data-path='{{$path}}' data-id="{{$record->$table_pk}}" data-toggle="modal" data-target="#delete" href="#">
  ...
  <!--begin of pagination -->
  <div class='pagination_wrapper col-lg-7 col-lg-push-3'>
    @if(!Request::input('q'))
    {!! $array->links() !!}
    @endif
  </div>
